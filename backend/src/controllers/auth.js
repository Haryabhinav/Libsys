...
export const registerUser = async (req, res, next) => {
    try {
        ...
        const accessToken = jwt.sign({ userId: savedUser._id }, process.env.JWT_ACCESS_SECRET, { expiresIn: '5s' });
        const refreshToken = jwt.sign({ userId: savedUser._id }, process.env.JWT_ACCESS_SECRET, { expiresIn: '1y' });
...
export const loginUser = async (req, res, next) => {
    try {
        ...
        // generate access token 
        const accessToken = jwt.sign({ userId: foundUser._id }, process.env.JWT_ACCESS_SECRET, { expiresIn: '20s' });
        const refreshToken = jwt.sign({ userId: foundUser._id }, process.env.JWT_ACCESS_SECRET, { expiresIn: '1y' });
